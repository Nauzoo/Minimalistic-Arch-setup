...
var require_graceful_fs = __commonJS((exports2, module2) => {
  ...
  module2.exports = patch(clone2(fs4));
...
var import_whatwg_url = __toModule(require_public_api());
...
async function getLatestRelease() {
  ...
  let response;
  try {
    response = await lib_default("https://marketplace.visualstudio.com/_apis/public/gallery/extensionquery ", {
      method: "POST",
      headers,
      body,
      timeout: 1e4
    ...
    import_coc.window.showErrorMessage(`[coc-sumneko]: Request failed! Please check your internet!`);
    console.error(err);
    return;
  }
  if (!response.ok) {
...
async function downloadServer(context, release) {
  ...
  const binPath = import_path.default.join(targetPath, "extension", "server", "bin", "lua-language-server");
...
function install(ctx) {
  return async () => {
    ...
    if (ctx.client && ctx.client.needsStop()) {
      ...
      import_coc2.window.showInformationMessage("May be you should restart the server");
    }
  };
}
function version(ctx) {
  return async () => {
    ...
    import_coc2.window.showInformationMessage(v);
  };
}
function showTooltip(ctx) {
  return async () => {
    import_coc2.window.showNotification({content: ctx.barTooltip});
  };
}
function insertNvimLuaPluginLibrary(_) {
  return async () => {
    const config = import_coc2.workspace.getConfiguration("Lua.workspace");
    const library = Array.from(config.get("library") || []);
    const runtimepath = await import_coc2.workspace.nvim.getOption("runtimepath");
    const paths = runtimepath.split(",").map((v) => {
      return import_path2.default.join(v, "lua");
    }).filter((v) => !library.includes(v) && (0, import_fs_extra.existsSync)(v));
    if (!paths.length)
      return;
    const vimruntime = await import_coc2.workspace.nvim.call("expand", ["$VIMRUNTIME/lua"]);
    const myvimrc = import_path2.default.join(import_path2.default.dirname(await import_coc2.workspace.nvim.call("expand", ["$MYVIMRC"])), "lua");
    const idx = await import_coc2.window.showQuickpick(paths.map((v, i) => {
      let name = "";
      if (v == vimruntime) {
        ...
        name = import_path2.default.basename(import_path2.default.dirname(v));
      }
...
function downloadNvimLuaTypes(ctx) {
  return async () => {
...
var fs3 = __toModule(require_lib());
...
var Config = class {
  ...
  get cfg() {
    return import_coc3.workspace.getConfiguration(this.rootSection);
  }
  get enabled() {
  ...
  get nvimLuaDev() {
    return this.cfg.get("enableNvimLuaDev");
  }
  get checkUpdate() {
    return this.cfg.get("checkUpdate");
  }
  get serverDir() {
    return this.cfg.get("serverDir");
  }
};
...
var Neodev = class {
  ...
  async downloadTypes() {
    import_coc4.window.showInformationMessage("Start downloading nvim lua types");
    await fs2.remove(this.repoDir);
    await (0, import_util.promisify)(import_child_process.exec)(`git clone https://github.com/folke/neodev.nvim.git ${this.repoDir} --depth=1`);
    if (this.typesExist) {
      import_coc4.window.showInformationMessage(`Download nvim lua types successfully!`);
    } else {
      import_coc4.window.showErrorMessage("Download nvim lua types failed!");
    }
  }
  get repoDir() {
    return import_path3.default.join(this.context.storagePath, typesDirName);
  }
  get typesExist() {
    return fs2.existsSync(this.repoDir);
  }
  async getTypesPath() {
    ...
    if (this.typesExist) {
      return import_path3.default.join(this.repoDir, "types", import_coc4.workspace.isNvim && Boolean(import_coc4.workspace.nvim.call("luaeval", "vim.version().prerelease")) ? "nightly" : "stable");
    } else {
      import_coc4.window.showWarningMessage("Please run command: installNvimLuaTypes");
      return "";
    }
  }
};
...
// src/ctx.ts
function isLuaDocument(document2) {
  const ret = document2.languageId === "lua";
  return ret;
}
var Ctx = class {
  constructor(extCtx) {
  ...
  resolveBin() {
    const serverDir = this.config.serverDir ? this.config.serverDir : import_path4.default.join(this.extCtx.storagePath, "sumneko-lua-ls", "extension", "server");
    const platform2 = process.platform;
    const bin = import_path4.default.join(serverDir, "bin", platform2 === "win32" ? "lua-language-server.exe" : "lua-language-server");
    if (!fs3.existsSync(bin)) {
      return;
    }
    if (!import_executable.default.sync(bin)) {
      import_coc5.window.showInformationMessage(`${bin} is not executable`, "error");
      return;
    }
    const args = ["-E", import_path4.default.join(serverDir, "bin", "main.lua"), `--locale=${this.config.locale}`].concat(import_coc5.workspace.getConfiguration("Lua").get("misc.parameters"));
  ...
  async getCurrentVersion() {
    if (this.config.serverDir) {
      ...
      args.push("--version");
      try {
        return String((0, import_child_process2.execSync)(`${cmd} ${args.join(" ")}`)).trim();
      } catch (err) {
        console.log(err);
        return;
      }
      ...
      try {
        const packageJson = import_path4.default.join(this.extCtx.storagePath, "sumneko-lua-ls", "extension", "package.json");
        const packageData = await fs3.readJson(packageJson);
  ...
  async checkUpdate() {
    ...
    if ((0, import_node_version_compare.default)(latestVersion[0], currentVersion) <= 0) {
      return;
    }
    const msg = `Sumneko lua-language-server has a new release: ${latest.version}, you're using v${currentVersion}.`;
    if (this.config.prompt) {
      const ret = await import_coc5.window.showQuickpick(["Download the latest server", "Cancel"], msg);
      if (ret === 0) {
        ...
        try {
          ...
          console.error(e);
          import_coc5.window.showInformationMessage("Upgrade server failed", "error");
          return;
        }
        this.client.start();
      } else {
        import_coc5.window.showInformationMessage(`You can run ':CocCommand sumneko-lua.install' to upgrade server manually`);
      }
    } else {
      import_coc5.window.showInformationMessage(`${msg} Run :CocCommand sumneko-lua.install to upgrade`);
    }
  }
  createClient() {
    const bin = this.resolveBin();
      ...
      middleware: {
        workspace: {
          configuration: async (params, token, next) => {
            ...
            const runtime = await import_coc5.workspace.nvim.call("expand", ["$VIMRUNTIME/lua"]);
    ...
    return new import_coc5.LanguageClient("sumneko-lua", "Sumneko Lua Language Server", serverOptions, clientOptions);
  }
  async startServer() {
  ...
  activateStatusBar() {
    const bar = import_coc5.window.createStatusBarItem();
    ...
    import_coc5.events.on("BufEnter", async () => {
    ...
    }, null, this.extCtx.subscriptions);
  }
  activateCommand() {
    this.client.onNotification("$/command", (params) => {
      ...
      for (const data of params.data) {
        const folder = import_coc5.workspace.getWorkspaceFolder(data.uri);
        const config = import_coc5.workspace.getConfiguration(void 0, folder ? data.uri : void 0);
        if (data.action == "add") {
...
async function activate(context) {
  ...
  if (!(0, import_fs_extra2.existsSync)(serverRoot)) {
    (0, import_fs_extra2.mkdirSync)(serverRoot);
  }
  ...
  if (!bin) {
    ...
    if (ctx.config.prompt) {
      ret = await import_coc6.window.showInformationMessage("Sumneko lua language server is not found, install now?", "Yes", "Cancel") || "Yes";
    }
    if (ret == "Yes") {
      try {
        ...
        console.error(e);
        import_coc6.window.showInformationMessage("Download Sumneko lua language server failed", "error");
        return;
      }
    } else {
      import_coc6.window.showInformationMessage(`You can run ':CocCommand sumneko-lua.install' to install server manually or provide setting 'serverDir'`);
      return;
    }
  }
  ...
  ctx.registerCommand("restart", (ctx2) => {
    return async () => {
      import_coc6.window.showInformationMessage(`Reloading sumneko lua-language-server...`);
      ...
      import_coc6.window.showInformationMessage(`Reloaded sumneko lua-language-server`);
    };
  });
  ctx.registerCommand("showTooltip", showTooltip);
  ctx.registerCommand("insertNvimLuaPluginLibrary", insertNvimLuaPluginLibrary);
  ctx.registerCommand("checkUpdate", () => async () => await ctx.checkUpdate());
  ctx.registerCommand("downloadNvimLuaTypes", downloadNvimLuaTypes);
  await ctx.startServer();
  if (ctx.config.checkUpdate) {
    const dataPath = import_path5.default.join(context.storagePath, "checkUpdate");
    let lastCheck = 0;
    if ((0, import_fs_extra2.existsSync)(dataPath)) {
      lastCheck = Number((await (0, import_fs_extra2.readFile)(dataPath)).toString());
    }
    const now = Date.now();
    if (now - lastCheck > 4 * 60 * 60 * 1e3) {
      await ctx.checkUpdate();
      await (0, import_fs_extra2.writeFile)(dataPath, now.toString());
    }
  }
}
